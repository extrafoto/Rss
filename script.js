...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Adicionar evento de clique para abrir a notícia
    storiesContent.addEventListener('click', (e) => {
        // Verificar se o clique foi no centro da tela (não nas áreas de navegação)
        const rect = storiesContent.getBoundingClientRect();
        const clickX = e.clientX - rect.left;
        const width = rect.width;
        
        // Se o clique foi no centro (entre 30% e 70% da largura)
        if (clickX > width * 0.3 && clickX < width * 0.7) {
            if (stories[currentIndex] && stories[currentIndex].link) {
                window.open(stories[currentIndex].link, '_blank');
            }
        }
    });
});
